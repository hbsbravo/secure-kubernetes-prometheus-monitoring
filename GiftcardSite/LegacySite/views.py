...
database_error_return_404 = Counter(
    'database_error_return_404',
    'Number of times a 404 was returned due to database errors'
)
...
def use_card_view(request):
    ...
    if request.method == 'GET':
        ...
        user_cards = Card.objects.raw(
            'select id, count(*) as count from LegacySite_card where LegacySite_card.user_id = %s' % str(request.user.id)
        )
        card_query_string = ""
